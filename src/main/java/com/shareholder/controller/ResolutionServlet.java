...
public class ResolutionServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            var resolutions = resolutionDAO.findAll();
            Map<Integer, Map<String, Integer>> voteResults = new LinkedHashMap<>();
            for (Resolution resolution : resolutions) {
                Map<String, Integer> result = resolutionService.getVoteResult(resolution.getResolutionId());
                result.put("TOTAL", result.values().stream().mapToInt(Integer::intValue).sum());
                voteResults.put(resolution.getResolutionId(), result);
            }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            if ("delete".equals(req.getParameter("action"))) {
                resolutionService.deleteUpcomingResolution(
                        Integer.parseInt(req.getParameter("resolutionId")), actorUserId, userAgent);
