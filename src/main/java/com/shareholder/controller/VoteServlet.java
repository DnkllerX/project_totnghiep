...
public class VoteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            // Chi hien nghi quyet dang OPEN - nghi quyet da qua han (CLOSED, kem ca cai da vote)
            // se tu an khoi danh sach nay theo yeu cau, khong con hien lai o day nua.
            List<Resolution> openOnly = resolutionDAO.findAll().stream()
                    .filter(r -> r.getStatus() == ResolutionStatus.OPEN)
                    .filter(r -> !LocalDateTime.now().isBefore(r.getStartTime())
                            && !LocalDateTime.now().isAfter(r.getEndTime()))
                    .collect(Collectors.toList());
            req.setAttribute("resolutions", openOnly);
