...
public class ShareDAOImpl implements ShareDAO {
    ...
    @Override
    public Optional<Share> findByShareholderIdForUpdate(Connection conn, int shareholderId) throws SQLException {
        // WITH (UPDLOCK, ROWLOCK): khoa dong ngay tu luc SELECT (khong doi den UPDATE moi khoa),
        // giu nguyen tren dung connection/transaction dang chay ben ngoai (KHONG mo connection moi
        // nhu findByShareholderId(int) - do la nguyen nhan chinh gay ho race condition truoc day).
        String sql = "SELECT share_id, shareholder_id, quantity, updated_at FROM SHARES " +
                     "WITH (UPDLOCK, ROWLOCK) WHERE shareholder_id = ?";
    ...
    /**
     * FLOOR(quantity * ratio) tung dong roi SUM - dung dung cong thuc voi
     * ShareIssueDetailDAOImpl.generateFromSnapshot de con so xem truoc khop voi con so
     * se duoc sinh that su (neu khong co giao dich nao xay ra them tu luc nay den snapshot).
     */
    @Override
    public long estimateIssueQuantity(java.math.BigDecimal ratio) throws SQLException {
        String sql = "SELECT COALESCE(SUM(CAST(FLOOR(quantity * ?) AS BIGINT)), 0) AS total " +
                     "FROM SHARES WHERE quantity > 0";
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setBigDecimal(1, ratio.setScale(6, java.math.RoundingMode.DOWN));
            try (ResultSet rs = ps.executeQuery()) {
