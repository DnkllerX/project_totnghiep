...
@WebServlet({"/app/admin/documents", "/app/it/documents"})
...
public class DocumentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        String view = "IT".equals(role)
                ? "/WEB-INF/views/it/documents.jsp"
                : "/WEB-INF/views/admin/documents.jsp";
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if ("delete".equals(action)) {
            if (!"IT".equals(session.getAttribute("role"))) {
                resp.sendError(HttpServletResponse.SC_FORBIDDEN, "Chi IT moi duoc xoa tai lieu");
                return;
            }
            try {
                int documentId = Integer.parseInt(req.getParameter("documentId"));
                documentService.deleteDocument(documentId, actorUserId, userAgent);
            } catch (NumberFormatException e) {
                req.setAttribute("error", "documentId khong hop le");
            } catch (SQLException e) {
                getServletContext().log("Loi xoa tai lieu", e);
                req.setAttribute("error", "He thong dang gap su co, vui long thu lai sau");
            }
            resp.sendRedirect(req.getContextPath() + "/app/it/documents");
            return;
        }

        // Tu day tro xuong la luong UPLOAD - CHI danh cho ADMIN. IT truy cap duoc trang nay (de doc
        // va xoa) nhung KHONG duoc phep them tai lieu moi.
        if (!"ADMIN".equals(session.getAttribute("role"))) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, "Chi ADMIN moi duoc them tai lieu moi");
            return;
        }
