...
public class ShareIssueService {
    ...
    /**
     * Xoa mot dot co tuc CHI khi chua den thoi diem chot snapshot. Truoc moc nay,
     * he thong chua tao danh sach quyen/ky nhan nen co the huy an toan ma khong
     * lam anh huong den so huu co phan. Cac lich hen lien quan cung duoc xoa trong
     * cung transaction de scheduler khong xu ly mot dot da bi huy.
     */
    public void deleteUnopenedIssue(int issueId, int actorUserId, String userAgent)
            throws SQLException, ValidationException {
        Optional<ShareIssue> issueOpt = shareIssueDAO.findById(issueId);
        if (issueOpt.isEmpty()) throw new ValidationException("Khong tim thay dot co tuc");
        if (!issueOpt.get().getSnapshotDate().isAfter(LocalDateTime.now())) {
            throw new ValidationException("Khong the xoa dot da den thoi diem chot snapshot");
        }
        ...
        try {
            ...
            try (var ps = conn.prepareStatement(
                    "DELETE FROM SCHEDULED_EVENTS WHERE issue_id = ? AND status = 'PENDING'")) {
                ps.setInt(1, issueId);
                removedEvents = ps.executeUpdate();
            }
            if (removedEvents != 2) {
                throw new ValidationException("Khong the xoa dot nay vi scheduler da bat dau xu ly");
            }
            try (var ps = conn.prepareStatement("DELETE FROM SHARE_ISSUES WHERE issue_id = ?")) {
                ps.setInt(1, issueId);
                if (ps.executeUpdate() != 1) throw new ValidationException("Khong tim thay dot co tuc");
            }
            auditLogDAO.insert(conn, new AuditLog(AuditAction.DELETE, EntityType.ISSUE,
                    issueId, actorUserId, userAgent));
            conn.commit();
