...
public class ShareIssueDAOImpl implements ShareIssueDAO {
    ...
    private static final String SELECT_BASE =
            ...
            "issue_price, start_date, end_date, description, created_at FROM SHARE_ISSUES ";
    ...
    private ShareIssue mapRow(ResultSet rs) throws SQLException {
        ...
        BigDecimal price = rs.getBigDecimal("issue_price");
        i.setIssuePrice(price);
    ...
    public int insert(Connection conn, ShareIssue issue) throws SQLException {
        String sql = "INSERT INTO SHARE_ISSUES " +
                "(title, issue_type, issue_date, snapshot_date, share_quantity, issue_ratio, issue_price, " +
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            if (issue.getIssuePrice() != null) ps.setBigDecimal(7, issue.getIssuePrice());
            else ps.setNull(7, Types.DECIMAL);
            ps.setTimestamp(8, Timestamp.valueOf(issue.getStartDate()));
            ps.setTimestamp(9, Timestamp.valueOf(issue.getEndDate()));
            ps.setString(10, issue.getDescription());
            ps.executeUpdate();
