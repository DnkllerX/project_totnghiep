...
public class ResolutionService {
    ...
    /** Xoa nghi quyet chua bat dau va chua co bat ky la phieu nao. */
    public void deleteUpcomingResolution(int resolutionId, int actorUserId, String userAgent)
            throws SQLException, ValidationException {
        Optional<Resolution> resolutionOpt = resolutionDAO.findById(resolutionId);
        if (resolutionOpt.isEmpty()) throw new ValidationException("Khong tim thay nghi quyet");
        if (!resolutionOpt.get().getStartTime().isAfter(LocalDateTime.now())) {
            throw new ValidationException("Chi duoc xoa nghi quyet chua den thoi gian bat dau");
        }
        ...
        try {
            conn = DBConnection.getConnection();
            conn.setAutoCommit(false);
            try (var ps = conn.prepareStatement("SELECT 1 FROM VOTES WHERE resolution_id = ?")) {
                ps.setInt(1, resolutionId);
                try (var rs = ps.executeQuery()) {
                    if (rs.next()) throw new ValidationException("Nghi quyet da co phieu bieu quyet, khong the xoa");
                }
            }
            int removedEvents;
            try (var ps = conn.prepareStatement(
                    "DELETE FROM SCHEDULED_EVENTS WHERE resolution_id = ? AND status = 'PENDING'")) {
                ps.setInt(1, resolutionId);
                removedEvents = ps.executeUpdate();
            }
            if (removedEvents != 1) {
                throw new ValidationException("Khong the xoa nghi quyet vi scheduler da bat dau xu ly");
            }
            try (var ps = conn.prepareStatement("DELETE FROM RESOLUTIONS WHERE resolution_id = ?")) {
                ps.setInt(1, resolutionId);
                if (ps.executeUpdate() != 1) throw new ValidationException("Khong tim thay nghi quyet");
            }
            auditLogDAO.insert(conn, new AuditLog(AuditAction.DELETE_RESOLUTION, EntityType.RESOLUTION,
                    resolutionId, actorUserId, userAgent));
            conn.commit();
    ...
    public void castVote(int resolutionId, int shareholderId, int snapshotId, VoteValue value,
        ...
        if (now.isBefore(resolution.getStartTime()) || now.isAfter(resolution.getEndTime())) {
            throw new ValidationException("Ngoai thoi gian cho phep bieu quyet");
        }
        if (voteDAO.hasVoted(resolutionId, shareholderId)) {
            throw new ValidationException("Ban da bieu quyet cho nghi quyet nay roi");
        }
        // Chan tu day: co dong phai co trong danh sach chot quyen (SHARE_SNAPSHOT_DETAILS) tai dung
        // snapshot cua nghi quyet nay thi moi duoc vote - khong de vote "am tham khong duoc tinh"
        // o buoc tally (VoteDAOImpl.countByResolutionId JOIN snapshot detail se tu loai ho ra).
        if (snapshotDetailDAO.findBySnapshotAndShareholder(snapshotId, shareholderId).isEmpty()) {
            throw new ValidationException(
                    "Ban khong co trong danh sach chot quyen bieu quyet cho nghi quyet nay");
        }
